...
Ce que la norme garantit exactement, et rien de plus :

| Type | Taille minimale garantie |
|------|--------------------------|
| `char` | 8 bits |
| `short` | 16 bits |
| `int` | 16 bits |
...
Elle garantit aussi un **ordre** : `char` ≤ `short` ≤ `int` ≤ `long` ≤ `long long`. Un type ne peut pas être plus petit que celui qui le précède, mais deux types voisins peuvent avoir la même taille (`int` et `long` font tous deux 32 bits sur ARM Cortex-M).

Taille réelle selon la plateforme :

| Plateforme | `int` | `long` |
|------------|-------|--------|
| AVR (Arduino Uno) | 16 bits | 32 bits |
| ARM Cortex-M (32 bits) | 32 bits | 32 bits |
| Linux x86-64 (modèle LP64) | 32 bits | **64 bits** |
| Windows x64 (modèle LLP64) | 32 bits | 32 bits |

Conséquence : si tu écris `unsigned long` en pensant avoir 32 bits, ton code est faux sur Linux 64 bits.

### 3.2 Qui décide de la taille réelle : l'implémentation et l'ABI

La norme fixe les minimums et l'ordre. C'est **l'implémentation C** (le compilateur, ses bibliothèques et les conventions de la machine) qui décide comment chaque type de base est représenté sur une plateforme précise.

Ces conventions de la machine sont regroupées dans l'**ABI** (*Application Binary Interface*). L'ABI définit les règles que doivent suivre les programmes compilés pour communiquer entre eux sur une architecture et un système donnés, au niveau **binaire**. C'est ce qui permet à des morceaux de code compilés séparément (ton programme, une bibliothèque, le noyau) de fonctionner ensemble.

L'ABI définit notamment :

- **la taille et l'alignement des types** (`int`, `long`, pointeurs…) ;
- **la disposition des structures** en mémoire (ordre des champs, padding, voir §13.3) ;
- **la convention d'appel** : quels registres servent à passer les arguments, où va la valeur de retour, qui sauvegarde quels registres ;
- **l'interface avec le système** : format des exécutables (ELF par exemple) et manière de faire un appel système.

C'est donc **l'ABI** qui décide que `long` fait 32 ou 64 bits sur une plateforme donnée. Deux exemples :

| ABI | Plateforme | `long` | Passage des premiers arguments |
|-----|------------|--------|--------------------------------|
| **System V AMD64** | Linux x86-64 | 64 bits | registres `rdi`, `rsi`, `rdx`… |
| **AAPCS** | ARM 32 bits (Cortex-M) | 32 bits | registres `r0` à `r3` |

### 3.3 Types à taille fixe : `<stdint.h>` (à utiliser systématiquement)
...
`uint32_t` vient de `<stdint.h>` (C99). Il garantit un entier non signé d'**exactement 32 bits**, sans bits de padding. Son nom décrit la taille voulue, pas un type de base.

`unsigned long` dit « au moins 32 bits ». `uint32_t` dit « exactement 32 bits ».

#### Lire un `typedef` : un alias, pas une variable

Dans `<stdint.h>`, on trouve par exemple :

```c
typedef unsigned long   uint32_t;
//      ^^^^^^^^^^^^^   ^^^^^^^^
//      type existant   nouveau nom
```

`typedef` ne déclare pas de variable : il crée un **alias**, c'est-à-dire un nouveau nom pour un type qui existe déjà. Cette ligne veut dire : « sur cette machine, quand j'écris `uint32_t`, le compilateur comprend `unsigned long` ». Les deux noms désignent exactement le même type.

#### Comment `<stdint.h>` fait le lien avec les types de base

Demander un `uint32_t`, c'est demander un entier non signé d'exactement 32 bits. Il faut donc un type de base disponible qui satisfait cette demande sur la plateforme, et ce type peut être un `unsigned long`, un `unsigned int`, etc.

Ce choix n'est **pas fait au moment où tu compiles**. Il a été fait **à l'avance** par ceux qui ont écrit le `<stdint.h>` de ta plateforme (l'équipe du compilateur ou de la libc). Ils connaissent la taille de chaque type de base sur cette cible (fixée par l'ABI, §3.2), donc ils ont écrit le `typedef` en dur :

```c
/* stdint.h pour ARM Cortex-M (arm-none-eabi-gcc / newlib) */
typedef unsigned long uint32_t;   // long = 32 bits ici

/* stdint.h pour Linux x86-64 (glibc) */
typedef unsigned int  uint32_t;   // long = 64 bits, donc on prend int
```

Quand tu écris `uint32_t`, le compilateur lit simplement ce `typedef` et le remplace par le type de base correspondant. Il n'y a ni décision ni recherche de sa part. Toi, tu écris `uint32_t` partout, et ton code garde la même signification sur toutes les cibles.

Si aucun type de base ne fait exactement 32 bits sur une plateforme, `uint32_t` n'est tout simplement **pas défini** et ton code ne compile pas (voir plus bas).

Tu peux vérifier une hypothèse de taille à la compilation :

```c
#include <limits.h>   // CHAR_BIT

_Static_assert(sizeof(unsigned long) * CHAR_BIT == 32, "long n'est pas 32 bits ici");
```

#### Pourquoi c'est essentiel dans ce domaine

- **Registres mémoire-mappés** : un registre périphérique de STM32 fait 32 bits, donc on y accède avec `volatile uint32_t *`. C'est ce que font les en-têtes CMSIS (`__IO uint32_t CR;`), voir §16.
- **Protocoles et formats binaires** : un en-tête TCP, une trame CAN ou un format de fichier définit des champs de taille exacte.
- **Crypto** : SHA-256 et ChaCha20 travaillent sur des mots de 32 bits. Les rotations et les débordements modulo 2³² ne donnent le bon résultat que si le type fait vraiment 32 bits. Avec un `unsigned long` de 64 bits, le hash serait faux sans aucune erreur de compilation.
- **Sécurité** : beaucoup de vulnérabilités (integer overflow, troncature, mauvais calcul de taille avant un `memcpy`) viennent d'hypothèses fausses sur la taille des types. Les types à largeur fixe rendent ces hypothèses explicites.

#### Subtilités

- `uint32_t` est **optionnel** dans la norme : il n'existe que si la machine possède un type d'exactement 32 bits. C'est le cas partout en pratique, mais certains DSP exotiques ont des `char` de 16 ou 32 bits.
- La même famille propose deux autres variantes :
  - `uint_least32_t` : le plus petit type d'au moins 32 bits. Il existe toujours.
  - `uint_fast32_t` : le type d'au moins 32 bits le plus rapide sur la cible. Il peut faire 64 bits sur x86-64.
- Pour afficher un `uint32_t` de façon portable, utilise `<inttypes.h>` (voir aussi §3.6) :

  ```c
  printf("%" PRIu32 "\n", x);   // pas "%lu" ni "%u", qui dépendent de la plateforme
  ```

- Attention aux **promotions entières** : un `uint8_t` ou un `uint16_t` est converti en `int` dans une expression. Par exemple, avec `uint16_t a = 0xFFFF;`, le calcul `a * a` peut déborder un `int` signé, ce qui est un comportement indéfini (voir §3.4).

### 3.4 Signé vs non signé
...
Rappel : `typedef` crée seulement un **alias** (un nouveau nom) pour un type existant. `flux_t` et la structure anonyme sont le même type. C'est exactement le mécanisme utilisé par `<stdint.h>` pour définir `uint32_t` (voir §3.3).
